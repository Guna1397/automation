...
scenario_name = 'Patient(s) is existing within DSP'

@scenario('test_get_patient_list_valid.feature', str(scenario_name))
def test_JAYU_3349():
    pass
...
def prepare_autherization_token():
    ...
    try:
        ...
        logger.info("Practitioner data and patient data is successfully onboarded")
...
def send_an_api_request():
    logger.info('Step:2')
    global response_data1,response_data2,  response1, response2, token,valid_ID1
    ...
    try:
        ...
        endpoint1 = f"{baseurl}/{valid_ID1}/patients"
        logger.info(f"Practitioner API URL used to retrieve Patient for specific practitioner is : {baseurl}")
