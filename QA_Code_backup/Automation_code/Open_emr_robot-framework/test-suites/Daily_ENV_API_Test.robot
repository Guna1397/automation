...
Validate Patient details when search with MRN using MRN Search API
    ...
    And params is parsed in MRN SEARCH API
...
#        Append To File  ${API_Output_filePath}${TC_ID}.txt  Response Status Code: ${response.status_code}${\n}
    #    Append To File  ${API_Output_filePath}${TC_ID}.txt  Response Json: ${data}${\n}
        ...
        ${status1}   Run Keyword And Return Status    Should Be Equal    ${data["entry"][0]["resource"]["identifier"][-1]["value"]}      ${parameter_list["patient_params"]["dsep_id"]}
        Run Keyword If   ${status1}   Append To File  ${API_Output_filePath}${TC_ID}.txt   PASS : DSEP Patient id in the request and response are same - ${data["entry"][0]["resource"]["identifier"][-1]["value"]}${\n}
        Run Keyword If    not ${status1}   Append To File  ${API_Output_filePath}${TC_ID}.txt   FAIL : DSEP Patient id in the request and response are not same - ${data["entry"][0]["resource"]["identifier"][-1]["value"]}${\n}
    END
...
#        Append To File  ${API_Output_filePath}${TC_ID}.txt  Response Status Code: ${response.status_code}${\n}
    #    Append To File  ${API_Output_filePath}${TC_ID}.txt  Response Json: ${data}${\n}
        ...
        Run Keyword If   ${status0}   Append To File  ${API_Output_filePath}${TC_ID}.txt   PASS : Response code ${response.status_code} received${\n}
        Run Keyword If    not ${status0}   Append To File  ${API_Output_filePath}${TC_ID}.txt   FAIL : Expected response code 200 not received, instead "${response.status_code}" shown${\n}
        ${status1}  Run Keyword And Return Status    Should Be Equal     ${parameter_list["mrn_search_params"]["mrn"]}   ${data['resource']['entry'][0]["resource"]['identifier'][1]['value']}
        Run Keyword If   ${status1}    Append To File  ${API_Output_filePath}${TC_ID}.txt  PASS : The received response contains MRN - ${data['resource']['entry'][0]['resource']['identifier'][1]['value']} & is same in both URL and Response${\n}
        Run Keyword If   not ${status1}    Append To File  ${API_Output_filePath}${TC_ID}.txt  FAIL : The received response contains MRN - ${data['resource']['entry'][0]['resource']['identifier'][1]['value']} & is NOT same in both URL and Response${\n}
    END
    IF  "${session_status}" == "False"
        Append To File  ${API_Output_filePath}${TC_ID}.txt   FAIL : Expected response code 200 not received, instead "403" shown${\n}
    END
...
#        Append To File  ${API_Output_filePath}${TC_ID}.txt  Response Status Code: ${response.status_code}${\n}
    #    Append To File  ${API_Output_filePath}${TC_ID}.txt  Response Json: ${data}${\n}
        ...
        Run Keyword If   ${status0}   Append To File  ${API_Output_filePath}${TC_ID}.txt   PASS : Response code ${response.status_code} received${\n}
        Run Keyword If    not ${status0}   Append To File  ${API_Output_filePath}${TC_ID}.txt   FAIL : Expected response code 200 not received, instead "${response.status_code}" shown${\n}
        ${status1}  Run Keyword And Return Status    Should Be Equal     ${parameter_list["practitioner_params"]["id"]}   ${data['entry'][0]['resource']["identifier"][-1]['value']}
        Run Keyword If   ${status1}    Append To File  ${API_Output_filePath}${TC_ID}.txt  PASS : The received response contains Practitioner id - ${data['entry'][0]['resource']["identifier"][-1]['value']} & is same in both URL and Response${\n}
        Run Keyword If  not ${status1}    Append To File  ${API_Output_filePath}${TC_ID}.txt  FAIL : The received response contains Practitioner id - ${data['entry'][0]['resource']["identifier"][-1]['value']} & is NOT same in both URL and Response${\n}
    END
    IF  "${session_status}" == "False"
        Append To File  ${API_Output_filePath}${TC_ID}.txt   FAIL : Expected response code 200 not received, instead "403" shown${\n}
    END
...
200 response code is displayed
    Should be equal as strings    200    ${r_response}


Generate JWT Token
    Append To File  ${API_Output_filePath}${TC_ID}.txt  ********** JWT Token API Result *********${\n}
    Append To File  ${API_Output_filePath}${TC_ID}.txt   JWT Token API Base URL: ${parameter_list["jwt_url"]}${\n}
    ...
    ${session_status}=    Run Keyword And Return Status        Get On Session      GENERATE_JWT  /    headers=${headers}
    Log To Console     ${session_status}
    IF  "${session_status}" == "True"
        ${resp}=    Get On Session   GENERATE_JWT  /    headers=${headers}
        ${status_jwt}  Run Keyword And Return Status   Status Should Be    200    ${resp}
        Run Keyword If  ${status_jwt}    Append To File  ${API_Output_filePath}${TC_ID}.txt   PASS : JWT Token Generated${\n}
    ...
    END
    IF  "${session_status}" == "False"
        Append To File  ${API_Output_filePath}${TC_ID}.txt   FAIL : JWT Token Not Generated${\n}
    END
